...
(define (domain v27_d)
    ...
    (:requirements :conditional-effects :negative-preconditions :equality :adl)
    (:types
        course program num1 num2 - object
    ...
    (:action add_course_with_prerequisites
        :parameters (?c1 ?c2 - course ?s1 ?s2 - num2 ?n - num1)
        :precondition 
        (and 
            (current ?n)
            (succ2 ?s1 ?s2)
    ...
    (:action add_course_with_double_prerequisites
        :parameters (?c1 ?c2 ?c3 - course ?s1 ?s2 - num2 ?n - num1)
        :precondition 
        (and 
            (current ?n)
            (succ2 ?s1 ?s2)
    ...
    (:action add_course_with_triple_prerequisites
        :parameters (?c1 ?c2 ?c3 ?c4 - course  ?s1 ?s2 - num2 ?n - num1)
        :precondition 
        (and 
            (current ?n)
            (succ2 ?s1 ?s2)
    ...
    (:action add_course_with_quad_prerequisites
        :parameters (?c1 ?c2 ?c3 ?c4 ?c5 - course  ?s1 ?s2 - num2 ?n - num1)
        :precondition 
        (and 
            (current ?n)
            (succ2 ?s1 ?s2)
    ...
    (:action availability-capstone
        ...
        (and 
            ...
            (capstone-course ?c)
            (course-counts n4 s5)
            (not (available-mutual ?c ?n))
            (not (unavailable-mutual ?c ?n))
        )
        :effect 
        (and 
            (available-mutual ?c n2)    
        )
    )
    ...
    (:action availability-mutual-exclusive-init-n1
        ...
        (and 
            (current n1)
            (mutual-exclusive ?c)
            (not (available-mutual ?c n1))
            (not (unavailable-mutual ?c n1))
            (not (capstone-course ?c))
        )
        :effect 
        (oneof
            (available-mutual ?c n1)
            (unavailable-mutual ?c n1)
        )
    )
    (:action availability-mutual-exclusive
        ...
        (and 
            ...
            (succ1 ?n1 ?n2)
            (mutual-exclusive ?c)
            (not (available-mutual ?c ?n2))
            (not (unavailable-mutual ?c ?n2))
        )
        :effect 
        (and
            (when (unavailable-mutual ?c ?n1) (available-mutual ?c ?n2))
            (when (available-mutual ?c ?n1) (unavailable-mutual ?c ?n2))
        )
    )
